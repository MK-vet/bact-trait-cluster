...
class TestSignalRecovery:
    def test_consensus_matrix_within_vs_between(self, planted_matrix, true_labels):
        ...
        for i in range(len(true_labels)):
            for j in range(i + 1, len(true_labels)):
                (within if true_labels[i] == true_labels[j] else between).append(
                    M[i, j]
                )
    ...
    def test_cliff_delta_planted_features_large(self, planted_df, true_labels):
        ...
        c0 = cd[
            (cd["Cluster"] == 0) & (cd["Feature"].isin([f"feat_{i}" for i in range(4)]))
        ]
    ...
    def test_enrichment_z_planted_features_significant(self, planted_df, true_labels):
        ...
        planted = ez[
            (ez["Cluster"] == 0) & (ez["Feature"].isin([f"feat_{i}" for i in range(4)]))
        ]
...
class TestCLI:
    def test_help_exits_zero(self):
        r = subprocess.run(
            [sys.executable, "-m", "bacttraitcluster.cli", "--help"],
            capture_output=True,
        )
    ...
    def test_version_output(self):
        ...
        r = subprocess.run(
            [sys.executable, "-m", "bacttraitcluster.cli", "--version"],
            capture_output=True,
            text=True,
        )
    ...
    def test_self_check_passes(self):
        r = subprocess.run(
            [sys.executable, "-m", "bacttraitcluster.cli", "--self-check"],
            capture_output=True,
            text=True,
        )
        assert r.returncode == 0
