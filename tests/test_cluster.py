...
class TestConsensus:
    ...
    def test_consensus_matrix_shape(self, binary_matrix):
        ...
        M = consensus_matrix(binary_matrix, k=3,
                             algorithms=["agglomerative_hamming"],
                             n_runs=10, seed=42, n_jobs=1)
        assert M.shape == (50, 50)
        assert M.min() >= 0.0
        assert M.max() <= 1.0

    def test_consensus_labels_count(self, binary_matrix):
        from bacttraitcluster.clustering.consensus import consensus_matrix, consensus_labels
        M = consensus_matrix(binary_matrix, k=3,
                             algorithms=["agglomerative_hamming"],
                             n_runs=10, seed=42, n_jobs=1)
    ...
    def test_register_algorithm(self, binary_matrix):
        from bacttraitcluster.clustering.consensus import register_algorithm, consensus_matrix
        def dummy(X, k, seed):
            return np.random.RandomState(seed).randint(0, k, X.shape[0])
        register_algorithm("dummy", dummy)
        M = consensus_matrix(binary_matrix, k=2, algorithms=["dummy"],
                             n_runs=5, seed=42, n_jobs=1)
        assert M.shape == (50, 50)
...
class TestProfiling:
    ...
    def test_enrichment_z_detects_signal(self, binary_df):
        ...
        c0 = ez[(ez["Cluster"] == 0) & (ez["Feature"].isin(["feat_0", "feat_1", "feat_2", "feat_3"]))]
        assert (c0["Z"] > 0).all()
