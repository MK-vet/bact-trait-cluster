...
def _hungarian_match(a: np.ndarray, b: np.ndarray) -> pd.DataFrame:
    ...
    for i, j in zip(r, c):
        rows.append(
            {"Label_A": int(ua[i]), "Label_B": int(ub[j]), "Overlap": int(mat[i, j])}
        )
    return pd.DataFrame(rows)


def _assignment_confidence(
    M: np.ndarray, labels: np.ndarray, ids: List[str]
) -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        conf = (
            in_mean - out_best
            if (np.isfinite(in_mean) and np.isfinite(out_best))
            else np.nan
        )
        out.append(
            {
                "Strain_ID": ids[i],
                "Cluster": int(li),
                "InCluster_Mean": in_mean,
                "BestOther_Mean": out_best,
                "Confidence": conf,
            }
        )
...
def _phylo_purity(
    labels: np.ndarray, D: np.ndarray, n_perm: int, seed: int
) -> Tuple[float, float]:
...
class Pipeline:
    ...
    def _write_manifest(self, used_layers: List[LayerSpec]) -> None:
        ...
        manifest = {
            ...
            "config_sha256": sha256_file(self.out / "config_used.yaml")
            if (self.out / "config_used.yaml").exists()
            else None,
            "inputs": [
                {"name": sp.name, "path": sp.path, "sha256": sha256_file(sp.path)}
                for sp in used_layers
            ],
        }
    ...
    def run(self) -> Dict[str, pd.DataFrame]:
        ...
        if algos_missing:
            logger.warning(
                "Some clustering algorithms are unavailable and will be skipped: %s",
                algos_missing,
            )
        if not algos_used:
            raise ValueError(
                f"No valid clustering algorithms available. Requested={cc.algorithms}. Missing={algos_missing}"
            )
        ...
        for sp in c.layers:
            lr = load_layer_csv(
                ...
                id_col=sp.id_column,
                all_ids=all_ids,
                feature_col=sp.feature_column
                if sp.format == "long"
                else sp.feature_column,
            ...
            nonbin = [
                ...
                if not (
                    str(data[col].dtype).startswith("Int")
            ...
            data_qc, rep = qc_binary_features(
                ...
                min_prev=fqc.min_prev,
                max_prev=fqc.max_prev,
                max_missing_frac=fqc.max_missing_frac,
            )
        ...
        res["coverage"] = cov_df
        qc_df = (
            pd.concat(qc_reports, ignore_index=True) if qc_reports else pd.DataFrame()
        )
        if not qc_df.empty:
        ...
        for lname, data in layers.items():
            ...
            if data.shape[0] < 4 or data.shape[1] < 2:
                logger.warning(
                    "  Layer %s skipped (n=%d, p=%d after QC/missingness filters).",
                    lname,
                    data.shape[0],
                    data.shape[1],
                )
            ...
            if np.isnan(X.astype(float)).any():
                logger.warning(
                    "  Layer %s still has NA after filtering; dropping rows with NA.",
                    lname,
                )
            ...
            if not kr:
                logger.warning(
                    "  Layer %s skipped (k_range incompatible with n=%d).",
                    lname,
                    len(ids_layer),
                )
                continue
            ...
            stab = stability_path(
                ...
                algos_used,
                cc.n_consensus_runs,
                cc.n_stability_splits,
                cc.subsample_fraction,
                c.seed,
                c.n_jobs,
            )
            sdf = pd.DataFrame(
            ...
            if cp.shap_enabled:
                try:
                    sh = shap_importance(
                        data0, labels, cp.shap_n_background, cp.shap_bootstrap, c.seed
                    )
            ...
            if cp.tda_enabled:
                try:
                    tda = persistent_homology(
                        X, cp.tda_max_dim, cp.tda_n_subsample, c.seed
                    )
            ...
            if c.phylo_validation.enabled and c.phylo_validation.tree_path:
                try:
                    ...
                    if c.phylo_validation.id_match_required:
                        ...
                        if missing:
                            logger.warning(
                                "  Tree missing %d clustered IDs (showing up to 10): %s",
                                len(missing),
                                missing[:10],
                            )
                    ...
                    if len(keep) >= 4:
                        ...
                        purity, pp = _phylo_purity(
                            labs_keep, D, c.phylo_validation.n_perm, c.seed
                        )
                        pdf = pd.DataFrame(
                            [
                                {
                                    "Purity_between_over_within": purity,
                                    "Perm_P": pp,
                                    "N": len(keep),
                                }
                            ]
                    ...
                    logger.warning(
                        "  Biopython not installed — phylo validation skipped"
                    )
        ...
        for i in range(len(names)):
            for j in range(i + 1, len(names)):
                ...
                rows.append(
                    {
                        "Layer_A": names[i].replace("_Cluster", ""),
                        "Layer_B": names[j].replace("_Cluster", ""),
                        "N_shared": int(m.sum()),
                        "ARI": ari,
                        "NMI": nmi,
                        "VI": vi,
        ...
        for lname in layers.keys():
            ...
            if mpath.exists() and cpath.exists():
                try:
                    ...
                    if lname in layers:
                        layer_best_X[lname] = (
                            layers[lname].loc[ids_here].to_numpy(dtype=float)
                        )
        ...
        if len(layer_consensus) >= 2:
            try:
                ...
                for lname in layer_consensus:
                    ...
                    id_maps[lname] = ids_l
                    common_ids = (
                        set(ids_l) if common_ids is None else (common_ids & set(ids_l))
                    )
                ...
                if len(common_ids) >= 6:
                    ...
                    k_guess = int(
                        np.median(
                            [
                                len(
                                    np.unique(
                                        pd.read_csv(self.out / lyr / "clusters.csv")[
                                            "Cluster"
                                        ]
                                    )
                                )
                                for lyr in layer_consensus
                            ]
                        )
                    )
                    ...
                    _save(wdf, str(self.out / "layer_weights.csv"))
                    res["layer_weights"] = wdf
                    _save(
                        cka_df.reset_index().rename(columns={"index": "Layer"}),
                        str(self.out / "cka_matrix.csv"),
                    )
                        ...
                        [
                            {
                                ...
                                "k_guess": int(k_guess),
                                "CKA_mean_offdiag": float(
                                    np.nanmean(
                                        cka_df.values[
                                            np.triu_indices_from(cka_df.values, 1)
                                        ]
                                    )
                                )
                                if cka_df.shape[0] > 1
                    ...
                    _save(fusion_summary, str(self.out / "fused_mode_summary.csv"))
                    res["fused_mode_summary"] = fusion_summary
                    # candidate k by spectral clustering
                    for k in [
                        kk
                        for kk in sorted(set([2, 3, 4, 5, 6, k_guess]))
                        if kk < len(common_ids)
                    ]:
                        labels_candidates[k] = fused_spectral_clusters(
                            Kf, k, seed=c.seed
                        )
                        Xmdl = pd.concat(
                            [layers[n].loc[common_ids] for n in layers], axis=1
                        ).to_numpy(dtype=float)
                        Xmdl = np.where(
                            np.isnan(Xmdl),
                            np.nanmean(Xmdl, axis=0, keepdims=True),
                            Xmdl,
                        )
                        mdl = mdl_path_from_candidates(Xmdl, labels_candidates)
                    if not mdl.empty:
                        _save(mdl, str(self.out / "fused_mdl_path.csv"))
                        res["fused_mdl"] = mdl
                        k_star = int(mdl.sort_values("MDL").iloc[0]["k"])
                    else:
                        k_star = k_guess
                    fused_lab = labels_candidates.get(
                        k_star, fused_spectral_clusters(Kf, k_star, seed=c.seed)
                    )
                    fdf = pd.DataFrame({id_col: common_ids, "Fused_Cluster": fused_lab})
                    _save(fdf, str(self.out / "fused_clusters.csv"))
                    res["fused_clusters"] = fdf
                    # prediction strength on fused concatenated feature matrix
                    Xf = pd.concat(
                        [layers[n].loc[common_ids] for n in layers], axis=1
                    ).to_numpy(dtype=float)
                    ps = prediction_strength(
                        Xf, labels_candidates, n_splits=15, seed=c.seed
                    )
                    if not ps.empty:
                        _save(ps, str(self.out / "prediction_strength.csv"))
                        res["prediction_strength"] = ps
                    # partition info per layer vs fused
                    pi_rows = []
                    for lname in layers:
                        if set(common_ids).issubset(set(layers[lname].index)):
                            info = partition_info_summary(
                                fused_lab,
                                layers[lname].loc[common_ids].to_numpy(dtype=float),
                            )
                            info["Layer"] = lname
                            pi_rows.append(info)
                            fp = feature_partition_nmi(
                                layers[lname].loc[common_ids], fused_lab
                            )
                            if not fp.empty:
                                ...
                                _save(
                                    fp,
                                    str(
                                        self.out / f"{lname}_feature_partition_nmi.csv"
                                    ),
                                )
                    if pi_rows:
                        _save(
                            pd.DataFrame(pi_rows), str(self.out / "partition_info.csv")
                        )
            except Exception as e:
