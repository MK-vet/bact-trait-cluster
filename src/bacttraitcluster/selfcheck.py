...
def run_self_check(report_path=None):
    ...
    checks=[]
    required_modules=['bacttraitcluster.config','bacttraitcluster.pipeline','bacttraitcluster.io.loader']
    ...
    anti_salami = {"novelty_axis": "Consensus clustering + multi-view similarity fusion + stability diagnostics", "scope_boundary": "No pairwise association inference, no MDR ontology/hypergraph, no phylogenetic comparative model", "methodological_uniqueness": "Consensus/fusion/stability metrics (ARI/NMI/VI, silhouette, assignment confidence)", "primary_user_story": "Clustering/fusion/stability for binary multi-layer strain profiles", "recommended_softwarex_validation": ["runtime benchmark scaling (synthetic n,p)", "smoke test on real biological CSV/Newick with saved outputs", "reproducibility via run_manifest + config snapshot", "documented limitations and domain assumptions"]}
    out={
      'schema_version':'1.0',
