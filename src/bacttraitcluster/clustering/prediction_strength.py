...
def prediction_strength(X: np.ndarray, labels_by_k: dict[int,np.ndarray], n_splits: int=25, seed: int=42) -> pd.DataFrame:
    ...
    for k in sorted(labels_by_k):
        ...
        for b in range(n_splits):
            ...
            if cl_scores:
                cps.extend([{'k':k,'Split':b,'Cluster':i,'PS_cluster':v} for i,v in enumerate(cl_scores)])
                ps.append(float(np.min(cl_scores)))
        if ps:
            arr=np.asarray(ps)
            rows.append({'k':k,'PS_mean':arr.mean(),'PS_ci_lo':np.quantile(arr,0.025),'PS_ci_hi':np.quantile(arr,0.975),'B':len(arr)})
        if cps:
            pass
