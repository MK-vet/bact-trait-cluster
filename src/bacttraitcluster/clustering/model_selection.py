...
def mdl_binary_partition(X: np.ndarray, labels: np.ndarray):
    ...
    return {'MDL':L_data+L_model,'L_data':L_data,'L_model':L_model,'k':k,'n':n,'p':p}


def mdl_path_from_candidates(X: np.ndarray, labels_by_k: dict[int,np.ndarray]) -> pd.DataFrame:
    out=[]
    for k,lab in sorted(labels_by_k.items()):
        r=mdl_binary_partition(X, lab); out.append(r)
    df=pd.DataFrame(out)
    if not df.empty:
