...
def optimize_kernel_weights(
    kernels: dict[str, np.ndarray], k: int = 2, cka_df: pd.DataFrame | None = None
):
    names = list(kernels)
    Ks = [
        (np.asarray(kernels[n], float) + np.asarray(kernels[n], float).T) / 2
        for n in names
    ]
    L = len(Ks)
    if cka_df is None:
    ...
    Kf = fused(w)
    wdf = pd.DataFrame(
        {"Layer": names, "Weight": w, "CKA_mean": np.nanmean(cka_df.values, axis=1)}
...
def fused_spectral_clusters(Kf: np.ndarray, k: int, seed: int = 42) -> np.ndarray:
    ...
    try:
        ...
        lab = KMeans(n_clusters=int(k), random_state=int(seed), n_init=10).fit_predict(
            X
        )
