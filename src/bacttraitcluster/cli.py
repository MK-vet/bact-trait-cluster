...
def _build_parser():
    p = argparse.ArgumentParser(
        prog="bact-trait-cluster", description="Run bact-trait-cluster pipeline"
    )
    p.add_argument(
        "--config", default="examples/config.yaml", help="Path to YAML config"
    )
    p.add_argument(
        "--self-check",
        action="store_true",
        help="Run internal validation self-check and print JSON",
    )
    p.add_argument(
        "--self-check-report",
        default=None,
        help="Optional path to save self-check JSON",
    )
    p.add_argument(
        "--benchmark",
        action="store_true",
        help="Run synthetic benchmark and print JSON",
    )
    p.add_argument(
        "--benchmark-report", default=None, help="Optional path to save benchmark JSON"
    )
    p.add_argument("--version", action="store_true", help="Print version and exit")
    p.add_argument(
        "--reliability-preflight",
        action="store_true",
        help="Run reliability preflight before analysis",
    )
    p.add_argument(
        "--reliability-only",
        action="store_true",
        help="Run reliability preflight and exit (no analysis)",
    )
    p.add_argument(
        "--reliability-fail-fast",
        action="store_true",
        help="Abort when reliability quality_gate status is FAIL",
    )
    p.add_argument(
        "--reliability-sensitivity",
        action="store_true",
        help="Enable lightweight sensitivity mini-runs",
    )
    p.add_argument(
        "--reliability-config",
        default=None,
        help="YAML/JSON reliability thresholds preset",
    )
    p.add_argument(
        "--reliability-outdir",
        default=None,
        help="Output dir for reliability artifacts (default: <output_dir>/_reliability)",
    )
    p.add_argument(
        "--consistency-spec",
        default=None,
        help="YAML/JSON spec for cross-tool label consistency report",
    )
    p.add_argument(
        "--consistency-out",
        default=None,
        help="Output JSON path for consistency report",
    )
    return p
...
def main(argv=None):
    ...
    try:
        ...
        if isinstance(rep, dict):
            (outdir / "config_validation.json").write_text(
