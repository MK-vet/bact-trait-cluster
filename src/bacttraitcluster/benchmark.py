...
def run_benchmark(out_path=None, n_rows=300, p_values=(1000, 5000, 20000), seed=123):
    ...
    for p in p_values:
        ...
        t = time.perf_counter()
        prev = df.mean(0)
        informative = int(((prev > 0.01) & (prev < 0.99)).sum())
        qc = time.perf_counter() - t
        t = time.perf_counter()
        pairs, chk = _assoc(df)
        assoc = time.perf_counter() - t
        rows.append(
            {
                "n_rows": n_rows,
                "p": int(p),
                ...
                "qc_sec": round(qc, 4),
                "assoc2000pairs_sec": round(assoc, 4),
                "pairs_eval": pairs,
                "checksum": round(chk, 6),
            }
        )
...
def run_benchmark_paper_ready(
    out_dir, n_rows=300, p_values=(1000, 5000, 20000), seed=123
):
    from pathlib import Path
    import json
    import pandas as pd
    ...
    if not df.empty:
        ...
        plt.plot(
            df["p"], df["assoc2000pairs_sec"], marker="o", label="Assoc (2000 pairs)"
        )
