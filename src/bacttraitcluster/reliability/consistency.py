...
def _load_label_table(path: str | Path, label_col: str, id_col: Optional[str] = None) -> pd.DataFrame:
    df = read_table(path)
    ...
    if label_col not in df.columns:
        ...
        if len(candidates) == 1:
            ...
            raise ValueError(f"Label column '{label_col}' not found in {path}; candidates={candidates}")
...
def compare_labelings(
    ...
    for i, a in enumerate(names):
        for b in names[i+1:]:
            ...
            if n == 0:
                rows.append({
                    "tool_a": a, "tool_b": b, "n_overlap": 0,
                    "ari": np.nan, "nmi": np.nan, "cramers_v_bc": np.nan
                })
                continue
            ...
            rows.append({
                "tool_a": a, "tool_b": b, "n_overlap": int(n),
                "ari": float(ari), "nmi": float(nmi), "cramers_v_bc": float(cv)
            })
...
def build_consistency_report(
    ...
    for tool, spec in tool_label_specs.items():
        ...
        if not p.exists():
            ...
            continue
        try:
            tbl = _load_label_table(p, label_col=spec.get("label_col", "label"), id_col=spec.get("id_col"))
            if tbl["label"].nunique() <= 1:
                skipped[tool] = {"reason": "degenerate_labels", "path": str(p), "n_unique_labels": int(tbl["label"].nunique())}
                continue
            tables[tool] = tbl
        except Exception as e:
            skipped[tool] = {"reason": "parse_error", "path": str(p), "error": str(e)}
    pairwise = compare_labelings(tables) if len(tables) >= 2 else pd.DataFrame(
        columns=["tool_a", "tool_b", "n_overlap", "ari", "nmi", "cramers_v_bc"]
    )
    flags = []
    for _, row in pairwise.iterrows():
        if pd.notna(row["nmi"]) and row["nmi"] < 0.05:
            flags.append(f"Near-zero NMI between {row['tool_a']} and {row['tool_b']} (n={int(row['n_overlap'])})")
        if pd.notna(row["cramers_v_bc"]) and row["cramers_v_bc"] < 0.05:
            flags.append(f"Near-zero Cramér's V between {row['tool_a']} and {row['tool_b']} (n={int(row['n_overlap'])})")
    status = "PASS"
    if flags:
        status = "WARN"
    if len(tables) < 2:
        status = "WARN"
        flags.append("Insufficient non-degenerate label tables for cross-tool consistency comparison")
    report = {
        "status": status,
