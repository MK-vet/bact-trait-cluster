...
def _collect_table_inputs(cfg) -> Dict[str, str]:
    ...
    for attr in (
        ...
        if hasattr(cfg, attr) and getattr(cfg, attr, None):
            datasets.setdefault(
                attr.replace("_csv", "").replace("_", "").upper()
                if attr.endswith("_csv")
                else attr,
                str(getattr(cfg, attr)),
            )
    return datasets
...
def run_reliability_preflight(
    ...
    *,
    ...
    if not datasets:
        raise RuntimeError(
            "No tabular input datasets detected for reliability preflight"
        )
    ...
    for name, path in datasets.items():
        ...
        fi_map[name] = fi
        deg[name] = check_degeneracy(
            feature_table=df, id_col=pf.id_col, min_samples=min_samples
        )
        if run_sensitivity:
            try:
                ...
                s_df.to_csv(
                    sensitivity_dir / f"{name}_sensitivity_minirun.csv", index=False
                )
            except Exception as e:
                deg[name].setdefault("warnings", []).append(
                    f"sensitivity_minirun_failed: {e}"
                )
    ...
    qg = quality_gate(
        preflights, overlap_df=overlap, config=rel_cfg, degeneracy_reports=deg
    )
    ...
    manifest = build_run_manifest_extension(
        ...
        config_snapshot={
            ...
            "reliability_thresholds": {
                k: v for k, v in rel_cfg.items() if k != "notes"
            },
        },
        output_dir=outdir,
    )
    ...
    do_fail_fast = bool(
        fail_fast or rel_cfg.get("fail_fast_on_quality_gate_fail", False)
    )
    if do_fail_fast and status == "FAIL":
        raise RuntimeError(
            "Reliability quality gate FAIL: " + "; ".join(qg.get("failures", []))
        )
...
def run_consistency_from_spec(
    spec_path: str, out_path: str | None = None
) -> Dict[str, Any]:
